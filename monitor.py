...
class AStockTicker(Gtk.Window):
    ...
    def create_widgets(self):
        ...
        self.price_label.override_font(
            Pango.FontDescription("SimHei Bold 10"))  # Reduced font size
        self.price_label.override_color(Gtk.StateFlags.NORMAL,
                                        self.price_color)
        main_box.pack_start(self.price_label, True, True, 0)

        # Price change percentage
        self.change_label = Gtk.Label(label="--")
        self.change_label.override_font(
            Pango.FontDescription("SimHei 8"))  # Keep slightly smaller
        self.change_label.override_color(Gtk.StateFlags.NORMAL,
                                         self.price_color)
    ...
    def adjust_opacity(self, widget):
        ...
        dialog = Gtk.Dialog(title="Adjust Opacity",
                            transient_for=self,
                            flags=0)
        dialog.add_buttons(Gtk.STOCK_OK, Gtk.ResponseType.OK)
        scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.3, 1.0,
                                         0.1)
        scale.set_value(self.opacity)
        scale.connect("value-changed",
                      lambda s: setattr(self, 'opacity', s.get_value()))
